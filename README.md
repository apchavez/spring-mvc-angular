...
Este repo es la contraparte imperativa (thread-per-request) de **[spring-webflux-angular](https://github.com/apchavez/spring-webflux-angular)** — mismo dominio, mismos 12 endpoints REST, mismo frontend, mismo comportamiento de caché (cache-aside) y rate-limiting con Redis, pero un modelo de ejecución del backend completamente distinto: JDBC bloqueante sobre Tomcat en vez de R2DBC no bloqueante sobre Netty. Ver el README de ese repo para un contraste detallado de qué cambia realmente a nivel de código entre los dos.
...
| `deploy.yml` | Manual (`workflow_dispatch`) | Levanta un cluster `kind` efímero dentro del propio runner, hace `helm upgrade --install` del chart completo (incluye Postgres/Redis/Kafka), verifica el rollout y corre un smoke test contra `/actuator/health/readiness` — el cluster se destruye al final del job |
...
`deploy.yml` es solo manual y no depende de ningún cluster ni secreto externo: levanta su propio cluster `kind` efímero dentro del runner de GitHub Actions, despliega el chart completo ahí (Postgres/Redis/Kafka incluidos) con credenciales de smoke-test generadas en el propio workflow, verifica que los Deployments queden `Ready` y confirma con un curl a `/actuator/health/readiness`. Al terminar, destruye el cluster — no deja nada corriendo ni cuesta nada.

```bash
gh workflow run deploy.yml
```

No requiere secretos configurados — es autocontenido.

---

## Kubernetes

Asume que ya existe un cluster con `ingress-nginx` y una `StorageClass` por defecto. Para levantar uno local sin costo:

```bash
kind create cluster --name product-service-mvc
...
**Login real**: el frontend Angular (`web/src/app/auth/`) hace login vía `POST /api/v1/auth/login` contra un store de usuarios demo (`admin`/`Demo#Admin2026!` rol `ADMIN`, `user`/`Demo#User2026!` rol `USER`) y guarda el JWT resultante — ver [`api/README.md`](api/README.md#seguridad) para el detalle completo del endpoint y [`web/README.md`](web/README.md#autenticación) para el flujo del frontend (interceptor, guard de rutas, logout).
