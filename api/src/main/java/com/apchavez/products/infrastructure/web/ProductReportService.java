...
public class ProductReportService {
    ...
    public byte[] generatePdf() throws IOException {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            document.save(out);
            log.info("Reporte PDF generado — productos={}, valorTotal={}", products.size(), totalValue);
            return out.toByteArray();
        } catch (IOException e) {
            log.error("Fallo al generar reporte PDF — productos={}", products.size(), e);
            throw e;
        }
    }
    ...
    public byte[] generateExcel() throws IOException {
        ...
        try (SXSSFWorkbook workbook = new SXSSFWorkbook(100)) {
            ...
            workbook.write(out);
            workbook.dispose();
            log.info("Reporte Excel generado — productos={}, valorTotal={}", products.size(), totalValue);
            return out.toByteArray();
        } catch (IOException e) {
            log.error("Fallo al generar reporte Excel — productos={}", products.size(), e);
            throw e;
        }
    }
